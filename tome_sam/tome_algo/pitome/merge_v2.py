...
# --------------------------------------------------------
# Remove low energy tokens from src
...
def generate_src_and_dst_idx(energy_score: torch.Tensor,
    ...
    then from the left tokens pick the ones with lowest energy score as the protected tokens, all others as src
    tokens.
    ...
    Args:
        ...
        r: number of reduced tokens == number of src tokens

    Returns:
        protected_idx, src_idx, dst_idx: three disjoint sets for protected, src and dst token indices, respectively
    """
    gather = mps_gather_workaround if energy_score.device.type == "mps" else torch.gather
    ...
    protected_and_src_idx = idx_buffer.argsort(dim=-1)[:, num_dst:] # (B, H*W - num_dst)
    r = min(r, protected_and_src_idx.shape[-1])
    protected_and_src_energy = gather(energy_score.view(B, H*W), dim=-1, index=protected_and_src_idx)
    sorted_indices = torch.argsort(protected_and_src_energy, dim=-1, descending=True)
    # tokens with lower energy should be protected, tokens with higher energy can be merged
    src_idx = gather(protected_and_src_idx, dim=-1, index=sorted_indices[:, :r]) #(B, r)
    protected_idx = gather(protected_and_src_idx, dim=-1, index=sorted_indices[:, r:])

    return protected_idx, src_idx, dst_idx
...
def pitome(
    ...
    B, T, _ = metric.shape
    H = W = int(T ** 0.5)
    protected_idx, a_idx, b_idx = generate_src_and_dst_idx(scores.view(B, H, W), r) # (B, T-num_dst), (B, num_dst)
    ...
    sim = gather(sim, dim=-2, index=a_idx.unsqueeze(-1).expand(B, r, num_dst))
    ...
    def merge(x: torch.Tensor, mode="mean") -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        dst = dst.scatter_reduce(dim=-2, index=dst_idx.unsqueeze(-1).expand(B, r, C),
                                 src=src, reduce=mode)
    ...
    def unmerge(x: torch.Tensor) -> torch.Tensor:
        _, _, c = x.shape
        protected, dst = x[:, :-num_dst, :], x[:, -num_dst:, :]
...
def pitome_vision_v2(
    ...
    with torch.no_grad():
        ...
        # print(energy_score[0].max(), energy_score[0].min(), energy_score[0].mean())
